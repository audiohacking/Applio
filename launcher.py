...
def setup_environment():
    ...
    os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = (
        "1"  # Enable Metal Performance Shaders with CPU fallback
    )
    os.environ["PYTORCH_MPS_HIGH_WATERMARK_RATIO"] = (
        "0.0"  # Disable MPS memory limit to prevent OOM errors
    )
    os.environ["OMP_NUM_THREADS"] = (
        "1"  # Optimize for single-threaded performance on Apple Silicon
    )
    ...
    try:
        ...
        def redirect_when_ready():
            ...
            if wait_for_server(timeout=600):
                ...
                print(
                    "Server did not start in time. Check ~/Library/Logs/Applio/console.log"
                )
