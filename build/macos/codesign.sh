#!/bin/bash
# Code signing script for Applio macOS app
...
find "$APP_PATH/Contents" -type f \( -name "*.dylib" -o -name "*.so" \) -print0 2>/dev/null | while IFS= read -r -d '' file; do
    sign_item "$file"
done
...
if [ -f "$APP_PATH/Contents/MacOS/Applio" ]; then
    sign_item "$APP_PATH/Contents/MacOS/Applio"
fi

if [ -f "$APP_PATH/Contents/MacOS/Applio_bin" ]; then
    sign_item "$APP_PATH/Contents/MacOS/Applio_bin"
fi
